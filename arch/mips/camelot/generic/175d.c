...
//	printk(KERN_ERR "\n IP175d IOCTL called! (%08x)\n",cmd);
	...
	{
	case	SIOCGPVALSET:
		iptr = (int *)ifr->ifr_data;
		//printk(KERN_ERR"ValSet: %d", *iptr);
		GPREG(GPVAL) = *iptr;
		return 0;
		break;
	case	SIOCGPVALGET:
		iptr = (int *)ifr->ifr_data;
		*iptr = GPREG(GPVAL);
		//printk(KERN_ERR"ValGet: %d",*iptr);
		return 0;
		break;
	case	SIOCGPDIRSET:
		iptr = (int *)ifr->ifr_data;
		//printk(KERN_ERR"DirSet: %d",*iptr);
		GPREG(GPDIR) = *iptr;
		return 0;
		break;
	case	SIOCGPDIRGET:
		iptr = (int *)ifr->ifr_data;
		*iptr = GPREG(GPDIR);
		//printk(KERN_ERR"DirGet: %d",*iptr);
		return 0;
		break;
	case	SIOCGPSELSET:
		iptr = (int *)ifr->ifr_data;
		//printk(KERN_ERR"SelSet: %d",*iptr);
		GPREG(GPSEL) = *iptr;
		return 0;
		break;
	case	SIOCGPSELGET:
